...
public class DataDicController {
    ...
     * @retrun void
     */
    @RequestMapping(value="findHealthExamDictionary",method=RequestMethod.POST)
    public void findHealthExamDictionary(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try{
            String resPString = dataDicService.findHealthExamDictionary(request);
            GzipUtil.write(response, resPString);
        }catch(Exception e){
            ReturnResult re = new ReturnResult();
            re.setState(2);
            re.setMessage(SystemUtils.getValue(Constants.GET_DATA_EXCEPTION));
            logger.error("查询所有字典【健康体检】异常:"+e);
            response.getWriter().write(JSON.toJSONString(re));
        }
    }
