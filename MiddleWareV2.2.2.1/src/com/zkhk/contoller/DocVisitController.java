...
public class DocVisitController {
    ...
    public void findCanVisitDiabeteMem(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try{
            ...
            re.setState(2);
            re.setMessage(SystemUtils.getValue(Constants.GET_DATA_EXCEPTION));
            logger.error("医生获取可进行糖尿病随访的会员异常:"+e);
            response.getWriter().write(JSON.toJSONString(re));
        }
    }
    ...
     * @retrun void
     */
    @RequestMapping(value="findSomeDatasByMemberId",method=RequestMethod.POST)
    public void findSomeDatasByMemberId(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try{
            String resPString = docVisitService.findSomeDatasByMemberId(request);
            GzipUtil.write(response, resPString);
        }catch(Exception e){
            ReturnResult re = new ReturnResult();
            re.setState(2);
            re.setMessage(SystemUtils.getValue(Constants.GET_DATA_EXCEPTION));
            logger.error("根据会员id自动获取最新的体征，血糖，用药等数据异常:"+e);
            response.getWriter().write(JSON.toJSONString(re));
        }
    }
